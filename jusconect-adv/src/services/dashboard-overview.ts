...
function sortTasksByUrgency(
  ...
  const firstDueTime = parseDateOnly(first.due_date)?.getTime() ?? Number.MAX_SAFE_INTEGER;
  const secondDueTime = parseDateOnly(second.due_date)?.getTime() ?? Number.MAX_SAFE_INTEGER;
...
export async function getDashboardOverview(): Promise<DashboardOverviewResult> {
  ...
  const latestMessages: DashboardOverviewMessage[] = limitItems(messages, 5).map(
    (message) => {
  ...
  return {
    ...
    summary: {
      ...
      overdue_tasks: pendingTasks.filter((task) => getIsOverdue(task.due_date))
        .length,
