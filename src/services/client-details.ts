...
export async function getClientDetails(
  ...
  if (casesError) {
    throw new Error("Não foi possível carregar os casos do cliente.");
  }

  const cases: CaseRow[] = casesData ?? [];
  const caseIds = uniqueStrings(cases.map((caseItem) => caseItem.id));
  ...
  if (caseIds.length > 0) {
    ...
    if (messagesError) {
      throw new Error("Não foi possível carregar as mensagens do cliente.");
    }
    ...
    if (documentsError) {
      throw new Error("Não foi possível carregar os documentos do cliente.");
    }
  ...
  const formattedCases: ClientDetailsCase[] = cases.map((caseItem) => ({
    id: caseItem.id,
  ...
  const recentMessages: ClientDetailsMessage[] = limitItems(
    messages
      .map((message) => {
  ...
  const recentDocuments: ClientDetailsDocument[] = limitItems(
    documents
      .map((document) => {
        const relatedCase = casesById.get(document.case_id);
  ...
  return {
    ...
    summary: {
      ...
      new_cases: cases.filter((caseItem) => caseItem.status === "new").length,
      in_progress_cases: cases.filter(
        (caseItem) => caseItem.status === "in_progress",
      ).length,
      waiting_client_cases: cases.filter(
        (caseItem) => caseItem.status === "waiting_client",
      ).length,
      closed_cases: cases.filter((caseItem) => isClosedCase(caseItem.status))
        .length,
